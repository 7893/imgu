// /home/admin/imgu/sync-lambda/clear_data.mjs (Corrected: Removed duplicate functions)
...
// --- 带重试的 DynamoDB 批量写入函数 (保留文件原有的第一个定义) ---
...
async function clearSyncControlTable() {
    ...
    const deleteParams = {
        ...
        Key: {
            syncType: config.syncTypeToDelete
        }
    };
...
async function main() {
    ...
    console.log(`将要清理 DynamoDB 同步控制表: ${config.dynamoDbSyncControlTableName} 中的 '${config.syncTypeToDelete}' 记录`); // 更新提示
    ...
    if (answer.toLowerCase() === 'yes') {
        ...
        try {
            ...
            if (r2Success && metadataDbSuccess && syncDbSuccess) { // 检查所有步骤
                console.log('\n所有数据清理完毕！');
            } else {
                console.error('\n数据清理过程中遇到问题，请检查以上日志！');
                process.exitCode = 1;
...
// --- 这里不再包含 clearR2Bucket 和 sendBatchWriteWithRetry 的重复定义 ---
